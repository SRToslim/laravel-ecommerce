...
class HomeController extends Controller
{
    ...
    public function load_to_days_deal_section()
    {
        return view('frontend.partials.to_days_deal_products_section');
    }
    ...
    public function all_outlets(){
        $office = Outlet::get();
        // dd($office);
        return view('frontend.outlets', compact('office'));
    }

    public function outlet_details($id){

        // $outlet = OCTime::with('days')->where('outlet_id', $id)->get();
        $outlets = OCTime::where('outlet_id', $id)->get();
        $info = Outlet::find($id);
        $days = Day::get();
        $output="";
        $i = 1;
        foreach($days as $day){
            foreach($outlets as $out){
                if($out->day_id == $day->id){
                    if($out->open_time != null){
                        $output.='<tr><td>'.$i++.'</td><td>'.ucfirst($day->name).'</td><td>'.$out->open_time.' AM</td><td>'.$out->close_time.' PM</td></tr>';
                    }else{
                        $output.='<tr><td>'.$i++.'</td><td>'.ucfirst($day->name).'</td><td><strong class="text-danger">OffDay</strong></td><td></td></tr>';
                    }

                }
            }
        }
        return response()->json(['success'=>true, 'outlet'=>$output, 'days'=>$days, 'info'=>$info]);
    }
}
